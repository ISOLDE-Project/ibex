...
`endif
  ...
  task endSimulation(input int errors);
    ...
    if (errors != 0) begin
      $display("[ASIC SIM] @ t=%0t - Fail!", $time);
      $display("[ASIC SIM] @ t=%0t - errors=%08x", $time, errors);
    end else begin
      $display("[ASIC SIM] @ t=%0t - Success!", $time);
      $display("[ASIC SIM] @ t=%0t - errors=%08x", $time, errors);
    end
    $finish;
  endtask
  ...
  initial begin
    ...
    if (!$value$plusargs("STIM_INSTR=%s", stim_instr)) begin
      ...
      $display("[ASIC SIM] @ t=%0t: loading %0s into imemory", $time, stim_instr);
      $readmemh(stim_instr, asic_tb.i_aida_top.i_imemory.u_tcdm_mem.memory);
    end
    ...
    if (!$value$plusargs("STIM_DATA=%s", stim_data)) begin
      ...
      $display("[ASIC SIM] @ t=%0t: loading %0s into dmemory", $time, stim_data);
      $readmemh(stim_data, asic_tb.i_aida_top.i_dmemory.u_tcdm_mem.memory);
    end
