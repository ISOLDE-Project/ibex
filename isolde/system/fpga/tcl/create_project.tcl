...
# Vivado version this flow was last validated with. Override it per board by
# setting `set scripts_vivado_version <ver>` in board/xilinx_<board>.cfg.
if { ![info exists scripts_vivado_version] } {
  set scripts_vivado_version 2022.1
}
set current_vivado_version [version -short]

if { [string first $scripts_vivado_version $current_vivado_version] == -1 } {
  puts "CRITICAL WARNING: This flow was validated with Vivado $scripts_vivado_version but is running in $current_vivado_version"
}
...
# An IP generated by a different Vivado version comes in locked and will fail
# later during synthesis with an unhelpful message. Say so here instead.
foreach ip [get_ips] {
  if { [get_property IS_LOCKED $ip] } {
    puts "CRITICAL WARNING: IP '$ip' is locked (generated with another Vivado version)."
    puts "                  Regenerate it: make -C ips/$ip clean all"
  }
}
...
# Same story for verilog_define: `set_property` replaces the fileset property,
# and the bender part of vivado_synth.tcl runs last, so the fusesoc/ibex
# defines (FPGA_XILINX, RV32M, RV32B, RegFile) have to be merged back in here.
if { [info exists ibex_verilog_defines] } {
  set_property verilog_define \
      [concat [get_property verilog_define [current_fileset]] $ibex_verilog_defines] \
      [current_fileset]
