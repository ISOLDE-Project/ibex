...
module isolde_addr_shim #(
    ...
    isolde_tcdm_if.slave tcdm_master,  // Interface for CPU requests
    isolde_tcdm_if.master  tcdm_slave    // Interface for memory response
);
  ...
  assign addr_valid = (tcdm_master.req.addr >= START_ADDR) && (tcdm_master.req.addr <= END_ADDR);

  // Perform address translation only if the address is valid
  assign translated_addr = addr_valid ? (tcdm_master.req.addr - START_ADDR) : 32'hDEADBEEF;  // Invalid address return

  // Single always_comb block for both request forwarding and response handling
  always_comb begin
    if (addr_valid) begin
      // Forward request to slave when address is valid
      tcdm_slave.req = '{  // Assign each field individually
          req: tcdm_master.req.req,
          we: tcdm_master.req.we,
          be: tcdm_master.req.be,
          addr: translated_addr,  // Assign the translated address
          data: tcdm_master.req.data
      };
      tcdm_master.rsp = tcdm_slave.rsp;
    end else begin
      // Do not forward request to slave when address is invalid
      tcdm_slave.req  = null_dev.req;

      // Provide invalid response to master when address is invalid
      tcdm_master.rsp = null_dev.rsp;
    end
  end
