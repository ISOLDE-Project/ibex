// Copyleft 2024
// Copyright 2018 ETH Zurich and University of Bologna, see also CREDITS.md.
// Licensed under the Apache License, Version 2.0, see LICENSE for details.
// SPDX-License-Identifier: Apache-2.0


/**
 * ISOLDE Custom Instruction decoder
 *
 */

`include "prim_assert.sv"



module isolde_decoder
  import isolde_register_file_pkg::RegDataWidth, isolde_register_file_pkg::RegCount, isolde_register_file_pkg::RegSize, isolde_register_file_pkg::RegAddrWidth;
  import isolde_decoder_pkg::*;
  import isolde_register_file_pkg::*;
  import isolde_x_register_file_pkg::*;
(
    input logic clk_i,
    input logic rst_ni,

    // to/from controller
    input logic isolde_decoder_instr_exec_i,
    input logic isolde_decoder_instr_valid_i,
    input  logic [31:0]       isolde_decoder_instr_rdata_i,           // from IF-ID pipeline registers
    output logic isolde_decoder_illegal_instr_o,  // illegal instr encountered
    output logic isolde_decoder_busy_o,
    output logic isolde_decoder_stalled_o,
    //ISOLDE Register file interface
    output isolde_rf_raddr_t isolde_rf_raddr_o,
    output write_port_t isolde_rf_wp_o,
    //isolde_x_register_file_if.cpu x_rf_bus,
    output isolde_x_rf_addr_t x_rf_addr_o,
    isolde_fetch2exec_if.dec isolde_decoder_exec_bus
);

  // FSM states
  typedef enum logic [2:0] {
    BOOT,
    FETCH_COMPUTE,
    FETCH_REST,     // Fetch the remaining words for multi-word instructions
    WAIT_EXEC_GNT,
    DONE
  } state_t;

  state_t idvli_state, idvli_next;
  logic [4:0] rd;

  isolde_opcode_e isolde_opcode_d, isolde_opcode_q;
  isolde_decoded_t isolde_decoded_instr;

   logic [4:0][31:0] isolde_decoder_instr_batch;  // from IF-ID pipeline registers

  logic [2:0] vlen_instr_words_d, vlen_instr_words_q;  // Instruction length in words
  logic [2:0] read_ptr;
  logic isolde_decoder_illegal_instr_d, isolde_decoder_illegal_instr_q;

  logic isolde_decoder_fsm_guard;
  assign isolde_decoder_fsm_guard = isolde_decoder_instr_valid_i;
  //
 
  //assign isolde_decoder_exec_bus.isolde_decoder_illegal_instr = isolde_decoder_illegal_instr_o;

  assign isolde_decoder_illegal_instr_o = isolde_decoder_illegal_instr_q & isolde_decoder_illegal_instr_d;

  assign isolde_decoded_instr = decode_isolde_opcode(
      isolde_decoder_instr_rdata_i[6:0],  //opcode
      isolde_decoder_instr_rdata_i[14:12],  //nnn
      isolde_decoder_instr_rdata_i[31:25]  //func7
  );
  assign isolde_opcode_d = isolde_decoded_instr.opcode;
  assign vlen_instr_words_d = isolde_decoded_instr.vlen_words;

  always_comb begin


    isolde_decoder_illegal_instr_d = (isolde_opcode_d == isolde_opcode_invalid) ? 1 : 0;
  end

  always_ff @(posedge clk_i, negedge rst_ni) begin
    if (!rst_ni) begin
      idvli_state <= BOOT;
      read_ptr <= 0;
      rd <= '0;
      isolde_opcode_q <= isolde_opcode_invalid;
      vlen_instr_words_q <= 3'd0;
      isolde_rf_wp_o <= '0;
      isolde_decoder_illegal_instr_q <= 1;
      isolde_rf_raddr_o <= '0;
      isolde_decoder_exec_bus.isolde_opcode <= isolde_opcode_invalid;
      isolde_decoder_exec_bus.func3 <= '0;
      isolde_decoder_exec_bus.funct2 <= '0;
      isolde_decoder_exec_bus.isolde_decoder_instr <= '0;
      isolde_decoder_exec_bus.isolde_decoder_imm32 <= '0;
      isolde_decoder_exec_bus.isolde_decoder_imm32_valid <= '0;
      x_rf_addr_o <= '0;
    end else begin

      if (~isolde_decoder_instr_exec_i) begin
        read_ptr <= 0;
        idvli_state <= BOOT;
      end else begin
        if (isolde_decoder_fsm_guard) begin
          isolde_decoder_instr_batch[read_ptr]<=isolde_decoder_instr_rdata_i;
          idvli_state <= idvli_next;
          case (idvli_next)

            FETCH_COMPUTE: begin

              if (~isolde_decoder_illegal_instr_d) begin
                isolde_decoder_illegal_instr_q        <= 0;
                idvli_state                           <= FETCH_REST;
                read_ptr                              <= 1;
                rd                                    <= isolde_decoder_instr_rdata_i[11:7];
                isolde_opcode_q                       <= isolde_opcode_d;
                vlen_instr_words_q                    <= vlen_instr_words_d;
                //to exec
                isolde_decoder_exec_bus.isolde_opcode <= isolde_opcode_d;
                //isolde_decoder_exec_bus.rs2           <= isolde_decoder_instr_rdata_i[24:20];
                //isolde_decoder_exec_bus.rs1           <= isolde_decoder_instr_rdata_i[19:15];
                isolde_decoder_exec_bus.func3         <= isolde_decoder_instr_rdata_i[14:12];
                //isolde_decoder_exec_bus.rd            <= isolde_decoder_instr_rdata_i[11:7];
                if (1 == vlen_instr_words_d) begin
                  isolde_decoder_exec_bus.isolde_decoder_instr <= isolde_decoder_instr_rdata_i;
                  case (isolde_opcode_d)
                    isolde_opcode_R_type: begin
                      x_rf_addr_o[2] <= isolde_decoder_instr_rdata_i[24:20];  //rs2
                      x_rf_addr_o[1] <= isolde_decoder_instr_rdata_i[19:15];  //rs1
                      x_rf_addr_o[0] <= isolde_decoder_instr_rdata_i[11:7];  //rd    
                    end
                    isolde_opcode_redmule: begin
                      x_rf_addr_o[2] <= isolde_decoder_instr_rdata_i[31:27];  //rs3
                      x_rf_addr_o[1] <= isolde_decoder_instr_rdata_i[24:20];  //rs2
                      x_rf_addr_o[0] <= isolde_decoder_instr_rdata_i[19:15];  //rs1  
                    end
                    isolde_opcode_redmule_gemm1: begin
                      isolde_rf_raddr_o[0] <= isolde_decoder_instr_rdata_i[31:27];  //rs3
                      x_rf_addr_o[2] <= isolde_decoder_instr_rdata_i[24:20];  //rs2
                      x_rf_addr_o[1] <= isolde_decoder_instr_rdata_i[19:15];  //rs1
                      x_rf_addr_o[0] <= isolde_decoder_instr_rdata_i[11:7];  //rd    
                    end
                  endcase
                end  /*else isolde_decoder_stalled_o <= 0;*/

              end else begin
                isolde_decoder_illegal_instr_q <= 1;
                read_ptr                       <= 0;
              end
            end
            FETCH_REST: begin
              read_ptr <= read_ptr + 1;
              case (isolde_opcode_q)
                isolde_opcode_vle32_4: begin
                  load_quad_word();
                end
                isolde_opcode_gemm: begin
                  decode_64b_gemm();
                end
                isolde_opcode_redmule_gemm: begin
                  decode_redmule_gemm();
                end
                isolde_opcode_conv2d: begin
                  decode_conv_2d();
                end
              endcase

            end
          endcase
        end else begin  //if (isolde_decoder_fsm_guard)
          case (idvli_next)
            BOOT: begin
              read_ptr <= read_ptr + 1;
            end
            FETCH_COMPUTE: begin
              read_ptr <= 0;
              idvli_state <= isolde_decoder_exec_bus.isolde_exec_gnt ? DONE : FETCH_COMPUTE;
            end
            WAIT_EXEC_GNT: begin

              idvli_state <= DONE;
            end
          endcase
        end
      end
    end
  end

  always_comb begin
    isolde_decoder_busy_o = (isolde_opcode_d == isolde_opcode_invalid) ? 0 : 1;
    isolde_decoder_stalled_o = 0;
    isolde_decoder_exec_bus.isolde_exec_req = 0;
    idvli_next = idvli_state;
    case (idvli_state)
      BOOT: begin
        if (read_ptr == 3'h6) begin
          idvli_next = FETCH_COMPUTE;
        end
      end

      FETCH_COMPUTE: begin
        if (~isolde_decoder_illegal_instr_d)
          isolde_decoder_stalled_o = (1 == vlen_instr_words_d) ? 1 : 0;
      end

      FETCH_REST: begin


        if (vlen_instr_words_q == read_ptr) begin

          isolde_decoder_exec_bus.isolde_exec_req = 1;
          isolde_decoder_busy_o = isolde_decoder_fsm_guard ? 0 : 1;
          idvli_next = isolde_decoder_stalled_o ? WAIT_EXEC_GNT : FETCH_COMPUTE;
        end else begin
          isolde_decoder_busy_o = 1;
          isolde_decoder_exec_bus.isolde_exec_req = 0;
        end

      end
      DONE: begin
        isolde_decoder_exec_bus.isolde_exec_req = 0;
        isolde_decoder_stalled_o = 0;

        isolde_decoder_busy_o = isolde_decoder_fsm_guard ? 0 : 1;

        idvli_next = isolde_decoder_fsm_guard ? FETCH_COMPUTE : DONE;

      end

    endcase
  end

  task static load_quad_word;
    begin
      // if (3'h4 == read_ptr) begin
      //   isolde_rf_wp_o.addr <= isolde_decoder_instr_rdata_i[11:7];
      //   isolde_rf_wp_o.data[3] <= isolde_decoder_instr_batch[3];
      //   isolde_rf_wp_o.data[2] <= isolde_decoder_instr_batch[2];
      //   isolde_rf_wp_o.data[1] <= isolde_decoder_instr_batch[1];
      //   isolde_rf_wp_o.data[0] <= isolde_decoder_instr_batch_i[0];
      //   isolde_rf_wp_o.we <= 1'b1;
      // end else if (3'h3 == read_ptr) begin
      //   isolde_rf_wp_o.addr <= isolde_decoder_instr_rdata_i[11:7];
      //   isolde_rf_wp_o.data[3] <= isolde_decoder_instr_batch_i[0];
      //   isolde_rf_wp_o.data[2] <= isolde_decoder_instr_batch[1];
      //   isolde_rf_wp_o.data[1] <= isolde_decoder_instr_batch[2];
      //   isolde_rf_wp_o.data[0] <= 0;
      //   isolde_rf_wp_o.we <= 1'b1;
      // end else if (3'h2 == read_ptr) begin
      //   isolde_rf_wp_o.addr <= isolde_decoder_instr_rdata_i[11:7];
      //   isolde_rf_wp_o.data[3] <= isolde_decoder_instr_batch[1];
      //   isolde_rf_wp_o.data[2] <= isolde_decoder_instr_batch[1];
      //   isolde_rf_wp_o.data[1] <= 0;
      //   isolde_rf_wp_o.data[0] <= 0;
      //   isolde_rf_wp_o.we <= 1'b1;
      // end
    end
  endtask

  task static decode_conv_2d;
    /**
  (v4i32 QPR:$rd2),  ( IntOp (  iPTR     GPR:$rd1)
                                                  ,(  iPTR     GPR:$rs1)
                                                  ,(  v4i32    QPR:$rs3)
                                                  ,(  iPTR     GPR:$rs2)
                                                  ,(  v4i32    QPR:$rs4)
                                                  ,(  iPTR     GPR:$rs5)
                                                  ,(  v4i32    QPR:$rs6)
                                                  ,(  v4i32    QPR:$rs7)
                                                  
                          
                          )
  **/

    begin

      // if (3'h2 == read_ptr) begin
      //   //first 32 bits
      //   isolde_decoder_exec_bus.isolde_decoder_instr <= isolde_decoder_instr_batch_i[3];
      //   x_rf_addr_o[2] <= isolde_decoder_instr_batch_i[2][24:20];  //rs2
      //   x_rf_addr_o[1] <= isolde_decoder_instr_batch_i[2][19:15];  //rs1
      //   x_rf_addr_o[0] <= isolde_decoder_instr_batch_i[2][11:7];  //rd1    
      //   //
      //   x_rf_addr_o[3] <= isolde_decoder_instr_batch_i[1][29:25];  //rs5  
      //   isolde_rf_raddr_o[2] <= isolde_decoder_instr_batch_i[1][24:20];  //rs4
      //   isolde_rf_raddr_o[1] <= isolde_decoder_instr_batch_i[1][19:15];  //rs3
      //   isolde_rf_raddr_o[0] <= isolde_decoder_instr_batch_i[1][11:7];  //rd2    
      //   // 
      //   isolde_rf_raddr_o[4] <= isolde_decoder_instr_batch_i[0][24:20];  //rs7
      //   isolde_rf_raddr_o[3] <= isolde_decoder_instr_batch_i[0][19:15];  //rs6

      // end
    end
  endtask

  task static decode_64b_gemm;
    /**
                            (v4i32 QPR:$rd2),  ( IntOp   (  iPTR  GPR:$rd1)
                                                  ,(  iPTR     GPR:$rs1)
                                                  ,(  v4i32    QPR:$rs4)
                                                  ,(  iPTR     GPR:$rs2)
                                                  ,(  v4i32    QPR:$rs5)
                                                  ,(  iPTR     GPR:$rs3)
                                                  ,(  i32       imm:$transA)
                                                  ,(  i32       imm:$transB)                          
                          )
  **/
    begin
      // if (3'h1 == read_ptr) begin
      //   //first 32 bits
      //   x_rf_addr_o[2] <= isolde_decoder_instr_batch[0][24:20];  //rs2
      //   x_rf_addr_o[1] <= isolde_decoder_instr_batch[0][19:15];  //rs1
      //   x_rf_addr_o[0] <= isolde_decoder_instr_batch[0][11:7];  //rd1             
      //   // // extension 32 bits
      //   isolde_rf_raddr_o[1] <= isolde_decoder_instr_batch[1][24:20];  //rs5
      //   isolde_rf_raddr_o[0] <= isolde_decoder_instr_batch[1][19:15];  //rs4
      //   isolde_rf_wp_o.addr <= isolde_decoder_instr_batch[1][11:7];  //rd2  
      //   isolde_decoder_exec_bus.funct2 <= isolde_decoder_instr_batch[1][6:5];  //_ext_funct2 
      //   x_rf_addr_o[3] <= isolde_decoder_instr_rdata_i[4:0];  //rs3

      // end
    end
  endtask


  task static decode_redmule_gemm;

    begin
      if (3'h3 == read_ptr) begin
        //first 32 bits
        isolde_decoder_exec_bus.isolde_decoder_instr <= isolde_decoder_instr_batch[0];
        x_rf_addr_o[2] <= isolde_decoder_instr_batch[0][24:20];  //rs2
        x_rf_addr_o[1] <= isolde_decoder_instr_batch[0][19:15];  //rs1
        x_rf_addr_o[0] <= isolde_decoder_instr_batch[0][11:7];  //rd             
        // extension 32 bits
        isolde_decoder_exec_bus.isolde_decoder_imm32_valid <= 4'b0111;
        isolde_decoder_exec_bus.isolde_decoder_imm32[2] <= isolde_decoder_instr_rdata_i;
        isolde_decoder_exec_bus.isolde_decoder_imm32[1] <= isolde_decoder_instr_batch[2];
        isolde_decoder_exec_bus.isolde_decoder_imm32[0] <= isolde_decoder_instr_batch[1];
      end
    end
  endtask


endmodule
